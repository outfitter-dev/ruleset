...
function printLocal(rulesets: ListedRuleset[]): void {
  ...
  for (const ruleset of rulesets) {
    ...
    if (ruleset.providers?.length) {
      logger.info(`    ${chalk.dim('Providers:')} ${ruleset.providers.join(', ')}`);
...
async function runList(options: {
  ...
  try {
      ...
      if (availablePresets.length === 0) {
        ...
        for (const preset of availablePresets) {
          ...
          if (preset.rules.length > 0) {
            logger.info(
              `    ${chalk.dim('Rules:')} ${preset.rules.map((r: any) => r.name).join(', ')} (${preset.rules.length} total)`
            );
          }

          if (isInstalled && installRecord) {
            const installedCount = installRecord.installedRules.length;
            const installedDate = new Date(installRecord.installedAt).toLocaleDateString();
            logger.info(`    ${chalk.dim('Installed:')} ${installedCount} rules on ${installedDate}`);

            if (installRecord.lastUpdate) {
              const updateDate = new Date(installRecord.lastUpdate).toLocaleDateString();
    ...
    if (allRulesets.length === 0) {
      ...
      logger.info(chalk.dim('Or "rulesets install --preset <name>" to install from a preset'));
      logger.info(chalk.dim('Or "rulesets init" to create a local ruleset'));
    }

    await printProjectSources(projectContext);
      ...
      try {
        ...
        if (Object.keys(installedPresets).length > 0) {
          ...
          for (const [name, record] of Object.entries(installedPresets)) {
            const presetRecord = record as any;
            logger.info(
              `  ${chalk.green('•')} ${chalk.bold(name)}@${presetRecord.presetVersion}`
            );
            if (presetRecord.installedRules.length > 0) {
              logger.info(
                `    ${chalk.dim('Rules:')} ${presetRecord.installedRules.map((r: any) => r.name).join(', ')}`
              );
            }
          }
          logger.info('');
          logger.info(chalk.dim('Run "rulesets list --presets" to see all available presets'));
        }
      } catch {
...
function createGlobMatchers(
  ...
  return patterns.map((pattern) => picomatch(pattern, { dot: true, posixSlashes: true }));
}
  ...
  for (const rawSource of candidateSources) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const nameFromRule =
          (ruleSection?.name ?? frontmatter.name ?? basename(file)).toString();
...
async function printProjectSources(
  projectConfigResult: Awaited<ReturnType<typeof loadProjectConfig>>
): Promise<void> {
  ...
  for (const raw of new Set(sources)) {
    ...
    const status = existsSync(resolved) ? chalk.green('found') : chalk.yellow('missing');
  ...
  if (projectConfigResult.path) {
    ...
    const suffix = projectConfigResult.format ? ` (${projectConfigResult.format})` : '';
