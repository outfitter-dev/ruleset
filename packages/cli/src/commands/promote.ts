...
export function promoteCommand(): Command {
  return new Command('promote')
    ...
    .option('--partials-dir <dir>', 'Directory where partials live', DEFAULT_PARTIALS_DIR)
...
async function runPromote(source: string, options: PromoteOptions): Promise<void> {
  ...
  try {
    ...
    if (!sourceWithinBoundary) {
      throw new Error(`Source path '${source}' is outside the project workspace.`);
    }

    const partialsWithinReal = await isPathWithinBoundaryReal(partialsRoot, cwd);
    ...
    const partialName = options.name
      ...
      : slugify(basename(sourcePath).replace(/\.[^.]+$/, ''));
