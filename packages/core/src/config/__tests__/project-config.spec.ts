...
import { loadProjectConfig, type ProjectConfigResult } from "../project-config";
...
describe("loadProjectConfig", () => {
  ...
  it("throws a helpful error when the config violates the schema", async () => {
    const projectDir = await createTempProject(async (dir) => {
      const configDir = path.join(dir, ".ruleset");
      await mkdir(configDir, { recursive: true });
      const yaml = `version: 42\nrule:\n  template: "yes"\n`;
      await writeFile(path.join(configDir, "config.yaml"), yaml, "utf8");
    });

    await expect(
      loadProjectConfig({ startPath: projectDir })
    ).rejects.toThrowError(/Invalid project config/);
  });
