...
import { validateFrontmatterSchema, formatValidationErrors } from '../schema-validator';
...
describe('schema-validator', () => {
  describe('validateFrontmatterSchema', () => {
    ...
    it('should validate rule.globs must be array of strings', () => {
      const frontmatter = {
        rule: {
          globs: ['valid', 123, true],
        },
      };

      const errors = validateFrontmatterSchema(frontmatter);
      expect(errors).toHaveLength(1);
      expect(errors[0].path).toBe('rule.globs');
  ...
  describe('formatValidationErrors', () => {
    it('should format validation errors nicely', () => {
      ...
      expect(formatted[1]).toBe('rule.template must be a boolean (got: string)');
    });
